...
int main(int argc, char **argv)
{
    ...
    double error;
    double *u, *u_old, *tmp, *fYsquared, *fXsquared;
    int allocCount;
...
//    printf("Input mits - maximum solver iterations:\n");
    ...
    fYsquared = (double*)calloc(m, sizeof(double));
    fXsquared = (double*)calloc(n, sizeof(double));
//    printf("allocCount=%d u=%p u_old=%p\n", allocCount, u, u_old);
    
    if (u == NULL || u_old == NULL || fYsquared == NULL || fXsquared == NULL)
    {
        printf("Not enough memory for two %ix%i, one 1x%i and one 1x%i matrices\n", n+2, m+2, n, m);
        exit(1);
    }
    ...
    // Declarations that used to be at the start of 
    // one_jacobi_iteration function
    ...
    for (y = 1; y < (m+1); y++)
    {
        ...
        for (x = 1; x < (n+1); x++)
        {
            fX = xLeft + (x-1)*deltaX;
            fXsquared[x-1] = fX*fX;
            f = -alpha*(1.0-fXsquared[x-1])*(1.0-fYsquared[y-1]) - 2.0*(2.0-fXsquared[x-1]-fYsquared[y-1]);
            updateVal = (	(SRC(x-1,y) + SRC(x+1,y))*cx +
                            (SRC(x,y-1) + SRC(x,y+1))*cy +
                            SRC(x,y)*cc - f
                        )/cc;
