...
//    printf("allocCount=%d u=%p u_old=%p\n", allocCount, u, u_old);
    ...
    {
        printf("Not enough memory for two %ix%i matrices and a %ix%i matrix.\n", n+2, m+2, n, m);
        exit(1);
    }
    maxIterationCount = mits-1;
    ...
    for (y = 1; y < (m+1); y++)
    {
        ...
        for (x = 1; x < (n+1); x++)
        {
            FUPD(x-1,y-1) = -alpha*(1.0-fX*fX)*(1.0 - fY*fY) - 2.0*(1.0-fX*fX) - 2.0*(1.0 - fY*fY);
            updateVal = ((SRC(x-1,y) + SRC(x+1,y))*cx +
                          (SRC(x,y-1) + SRC(x,y+1))*cy +
                          SRC(x,y)*cc - FUPD(x-1,y-1))/cc;
...
//        printf("\tError %g\n", error);
    ...
    for (y = 1; y < (m+1); y++)
    {
        ...
        for (x = 1; x < (n+1); x++)
        {
            error += pow((SRC(x,y) - (1.0-fX*fX)*(1.0-fY*fY)), 2.0);
            fX += deltaX;
        }
        fY += deltaY;
    }
